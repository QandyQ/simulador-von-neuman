...
INSTRUCTIONS['init'] = [];
INSTRUCTIONS['init'][01] = function(){ showWire('CPro-RDir'); changeContent('CPro', 'RDir');};
INSTRUCTIONS['init'][02] = function(){ showWire('CProInc'); changeSpecialContent('CPro', countIncrement());};
INSTRUCTIONS['init'][03] = function(){ showWire('RDir-TD' + _c('RDir'));};
INSTRUCTIONS['init'][04] = function(){ showWire('TD-RDat' + _c('RDir')); changeContent('TD' + _c('RDir'), 'RDat');};
INSTRUCTIONS['init'][05] = function(){ showWire('RDat-RIns'); changeContent('RDat', 'RIns');};
INSTRUCTIONS['init'][06] = function(){ showWire('RIns-Deco'); changeDecoder(_t('RIns').substr(0,4));};
INSTRUCTIONS['init'][07] = function(){
...
INSTRUCTIONS['init'][08] = function(){ showWire('RDir-TD' + _c('RDir'));};
INSTRUCTIONS['init'][09] = function(){ showWire('TD-RDat' + _c('RDir')); changeContent('TD' + _c('RDir'), 'RDat');};
INSTRUCTIONS['init'][10] = function(){ showWire('RDat-REnt'); changeContent('RDat', 'REnt');};
INSTRUCTIONS['init'][11] = function(){ showWire('ALU-Acum'); changeSpecialContent('Acum', runALU(ACTUAL.ALU));};
INSTRUCTIONS['init'][12] = function(){ init();};

// Pasos de pasar acumulador a memoria
INSTRUCTIONS['0110'] = [];
INSTRUCTIONS['0110'][01] = function(){ showWire('RIns-RDir'); changeSpecialContent('RDir', _t('RIns').substr(4,4));};
INSTRUCTIONS['0110'][02] = function(){ showWire('RDir-TD' + _c('RDir'));};
INSTRUCTIONS['0110'][03] = function(){ showWire('RDir-TD' + _c('RDir')); showWire('Acum-RDat'); changeContent('Acum', 'RDat');};
INSTRUCTIONS['0110'][04] = function(){ showWire('RDat-TD' + _c('RDir')); showWire('RDir-TD' + _c('RDir')); TM[_c('RDir')] = _t('RDat'); setMemoryTable();};
INSTRUCTIONS['0110'][05] = function(){ init();};

// Pasos de parar la ejecución
INSTRUCTIONS['0111'] = [];
INSTRUCTIONS['0111'][01] = function(){ changeSpecialContent('CPro', '1111'); nextDoc(); ACTUAL.inst = 'finished';};

// Instrucciones de la ALU
// Nota: la entrada tiene que ser decimal
 
var ALU = [];
ALU['0000'] = function(a, b){return a + b;};
ALU['0001'] = function(a, b){return a - b;};
ALU['0010'] = function(a, b){return a * b;};
ALU['0011'] = function(a, b){return Math.pow(a, b);};
ALU['0100'] = function(a, b){return a & b;};
ALU['0101'] = function(a, b){return a | b;};
...
DOC['0111'] = [];
DOC['0111'][01] = "El *Decodificador* interpreta que se finaliza el programa y se para la ejecución.";
...
function runProgram(){
	...
	var infoReset = $('info_b_r');
	var infoAbout = $('info_b_a');
	var infoClose = $('info_b_c');
	var infoNext = $('info_b_n');
	if(infoReset) infoReset.addEventListener('click', function(){$('full').style.display = '';},false);
	if(infoAbout) infoAbout.addEventListener('click', about,false);
	if(infoClose) infoClose.addEventListener('click', hideDoc,false);
	if(infoNext) infoNext.addEventListener('click', buttonNextStep, false);
